...
{
    /// <summary>
    /// This class encapsulates all code that is unique to the AVT 838/841 interface.
    /// </summary>
    /// 
    public class AvtDevice2 : SerialDevice
    {
        public const string DeviceType = "AVT (838/841)";
        ...
        public static readonly Message AVT_838_IDLE             = new Message(new byte[] { 0x27 });       // 91 27
        public static readonly Message AVT_841_IDLE             = new Message(new byte[] { 0x12 });       // 91 12
        ...
        public AvtDevice2(IPort port, ILogger logger) : base(port, logger)
        {
        ...
        public override async Task<bool> Initialize()
        {
            ...
            await this.Port.Send(AvtDevice2.AVT_RESET.GetBytes());
            ...
            if (m.Status == ResponseStatus.Success)
            {
                switch (m.Value.GetBytes()[0])
                {
                    case 0x27:
                        ...
                        break;
                    case 0x12:
                        this.Logger.AddUserMessage("AVT 841 Reset OK");
                        this.Device = 2;
                        break;
                    default:
            ...
            switch (Device)
            {
                ...
                case 2:
                    ...
                    await this.Port.Send(AvtDevice2.AVT_ENTER_VPW_MODE.GetBytes());
                    ...
                    else
                    {
                        ...
                        this.Logger.AddDebugMessage("Expected " + AvtDevice2.AVT_VPW.ToString());
                        return false;
                    }
                    break;
            }
        ...
        protected override async Task<bool> SetVpwSpeedInternal(VpwSpeed newSpeed)
        {
            ...
            if (newSpeed == VpwSpeed.Standard)
            {
                ...
                await this.Port.Send(AvtDevice2.AVT_1X_SPEED.GetBytes());
            ...
            else
            {
                ...
                await this.Port.Send(AvtDevice2.AVT_4X_SPEED.GetBytes());
