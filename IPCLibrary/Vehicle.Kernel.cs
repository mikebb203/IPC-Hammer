...
namespace PcmHacking
{
    ...
    public partial class Vehicle : IDisposable
    {
        ...
        private async Task<Response<bool>> WriteBlock(byte block, byte[] data)
        {
            ...
            switch (data.Length)
            {
                case 6:
                    ...
                    break;
                case 5:
                    m = new Message(new byte[] { 0x6C, DeviceId.Pcm, DeviceId.Tool, 0x3B, block, data[0], data[1], data[2], data[3], data[4] });
                    break;
                case 3:
                    m = new Message(new byte[] { 0x6C, DeviceId.Pcm, DeviceId.Tool, 0x3B, block, data[0], data[1], data[2] });
                    break;
                case 2:
                    m = new Message(new byte[] { 0x6C, DeviceId.Pcm, DeviceId.Tool, 0x3B, block, data[0], data[1] });
                    break;
                default:
        ...
        public async Task<bool> PCMExecute1( int address, int claimedSize, CancellationToken cancellationToken)
        {
            // Note that we request an upload of 4k maximum, because the PCM will reject anything bigger.
            // But you can request a 4k upload and then send up to 16k if you want, and the PCM will not object.
            ...
            for (int attempt = 0; attempt < 5; attempt++)
            {
                logger.AddUserMessage("Requesting permission to upload.");
                await this.SetDeviceTimeout(TimeoutScenario.ReadProperty);
                Message request = protocol.CreateUploadRequest(address, claimedSize);
                if (!await TrySendMessage(request, "upload request"))
                {
                ...
                if (await this.WaitForSuccess(this.protocol.ParseUploadPermissionResponse1, cancellationToken))
                {
                    logger.AddUserMessage("Upload permission granted.");
                    uploadAllowed = true;
                    break;
            ...
            if (!uploadAllowed)
            {
                logger.AddUserMessage("Permission to upload was denied.");
        ...
        public async Task<bool> PCMStartProgram(int address, int claimedSize, CancellationToken cancellationToken)
        {
            // Note that we request an upload of 4k maximum, because the PCM will reject anything bigger.
            // But you can request a 4k upload and then send up to 16k if you want, and the PCM will not object.
            ///int claimedSize = 0x0400;
            ...
            for (int attempt = 0; attempt < 5; attempt++)
            {
                logger.AddUserMessage("Requesting permission to upload.");
                await this.SetDeviceTimeout(TimeoutScenario.ReadProperty);
                Message request = protocol.CreateStartProgramRequest(address, claimedSize);
                if (!await TrySendMessage(request, "Start program request"))
                {
                    return false;
                }

                if (await this.WaitForSuccess(this.protocol.ParseStartProgramResponse, cancellationToken))
                {
                    logger.AddUserMessage("Start program permission granted.");
                    uploadAllowed = true;
                    break;
                }
            ...
            if (!uploadAllowed)
            {
                logger.AddUserMessage("Permission to program was denied.");
                logger.AddUserMessage("If this persists, try cutting power to the IPC, restoring power, waiting ten seconds, and trying again.");
                return false;
            }
        ...
        public async Task<Response<bool>> WritePayload(Message message, CancellationToken cancellationToken)
        {
            ...
            for (; retryCount < MaxSendAttempts; retryCount++)
            {
                ...
                this.logger.AddDebugMessage("Upload request failed.");
                await Task.Delay(100);
